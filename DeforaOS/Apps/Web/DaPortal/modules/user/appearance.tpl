...
require_once('./system/user.php'); if(_user_admin($user_id)) { ?>
	...
	<?php } ?>
		...
		<tr>
			<td></td>
			<td><a href="<?php echo _html_link('user'); ?>"><button type="button" class="icon cancel"><?php echo _html_safe(CANCEL); ?></button></a> <input type="submit" value="<?php echo _html_safe(SUBMIT); ?>" class="icon submit"/></td>
		</tr>
	</table>
</form>
