...
<?php if(!isset($comment['preview'])) { ?>
		<div class="status">
			<a href="<?php echo _html_link($comment['module'], 'reply', $comment['parent'], FALSE, 'parent='.$comment['id']); ?>#edit"><?php echo _html_icon('reply'); ?> <?php echo _html_safe(REPLY); ?></a>
<?php global $user_id; require_once('./system/user.php');
if($comment['id'] && _user_admin($user_id)) { ?>
			<span class="middot">&middot;</span>
			<a href="<?php echo _html_link('content', 'update', $comment['id']); ?>"><?php echo _html_icon('edit'); ?> <?php echo _html_safe(EDIT); ?></a>
<?php } ?>
		</div>
<?php } ?>
	</div>
