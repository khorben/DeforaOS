...
class ArticleModule extends Module
{
	...
	//ArticleModule::_insert
	private function _insert($article)
	{
		global $user_id;
	...
	public function admin($args)
	{
		...
		print('<h1 class="title article">'._html_safe(
		...
		if(isset($args['sort']))
			switch($args['sort'])
			{
				...
				case 'enabled':
					$sort = 'daportal_content.enabled';
					break;
				case 'name':	$sort = 'title';	break;
				case 'date':
				default:	$order = 'DESC';	break;
			}
		$query = 'SELECT content_id AS id, timestamp'
			.', daportal_content.enabled AS enabled, title AS name'
	...
	//ArticleModule::_default
	public function _default($args)
	{
		if(isset($args['id']))
			return $this->display(array('id' => $args['id']));
		$this->_list($args);
	}


	private function article_delete($args)
	{
		global $user_id;
	...
	private function article_disable($args)
	{
		global $user_id;
	...
	private function article_enable($args)
	{
		global $user_id;
	...
	public function _list($args)
	{
		$title = ARTICLES;
		$where = '';
		$query = 'SELECT username FROM daportal_user'
			." WHERE user_id='".$args['user_id']."'";
		if(isset($args['user_id']) && ($username = _sql_single($query))
				!= FALSE)
		{
			$title = ARTICLES._BY_.' '.$username;
			$where = " AND daportal_content.user_id='"
				.$args['user_id']."'";
		}
		print('<h1 class="title article">'._html_safe($title)
				."</h1>\n");
		$query = 'SELECT content_id AS id, timestamp, title, content'
		...
		if(!isset($username))
		{
			...
			foreach($res as $article)
			{
				...
				_content_select_lang($article['id'],
						$article['title'],
						$article['content']);
				include('./modules/article/display.tpl');
			}
	...
	private function article_modify($args)
	{
		global $user_id;
	...
	private function article_submit($args)
	{
	...
	public function system($args)
	{
		...
		{
			case 'submit':
				return $this->_system_submit($args);
			case 'update':
				return $this->_system_update($args);
		}
	}
	...
	private function _system_submit($args)
	{
		...
		if(!($article['id'] = $this->_insert($article)))
		{
			$error = 'Could not insert article';
			return;
		}
		$this->_submit_send_mail($article);
		header('Location: '._module_link('article', 'submit', FALSE,
					FALSE, 'send='));
		exit(0);
	}

	private function _submit_send_mail($article)
	{
		//send mail
	...
	private function article_update($args)
	{
		global $error, $user_id, $user_name;
