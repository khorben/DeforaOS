...
<?php } ?>
	<table>
		...
		<tr><td></td><td><a href="<?php echo _html_link('project', isset($project['id']) ? FALSE : 'list', isset($project['id']) ? $project['id'] : FALSE); ?>"><button type="button" class="icon cancel"><?php echo _html_safe(CANCEL); ?></button></a> <input type="submit" value="<?php echo isset($project) ? _html_safe(UPDATE) : _html_safe(SEND); ?>" class="icon submit"/></td></tr>
	</table>
</form>
