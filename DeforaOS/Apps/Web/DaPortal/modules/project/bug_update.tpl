...
<form action="<?php echo _html_link(); ?>" method="post">
	...
	<input type="hidden" name="action" value="bug_<?php echo isset($bug['id']) ? 'update' : 'insert'; ?>"/>
<?php if(!isset($bug['id'])) { ?>
...
foreach($types as $t) { ?>
				<option value="<?php echo _html_safe($t); ?>"<?php if(isset($bug['type']) && $bug['type'] == $t) { ?> selected="selected"<?php } ?>><?php echo _html_safe($t); ?></option>
<?php } ?>
			</select></td></tr>
...
foreach($priorities as $p) { ?>
				<option value="<?php echo _html_safe($p); ?>"<?php if(isset($bug['priority']) && $bug['priority'] == $p) { ?> selected="selected"<?php } ?>><?php echo _html_safe($p); ?></option>
<?php } ?>
...
<?php } ?>
		...
		<tr><td></td><td><a href="<?php echo _html_link('project', 'bug_list', $project_id); ?>"><button type="button" class="icon cancel"><?php echo _html_safe(CANCEL); ?></button></a> <button type="submit" name="preview" class="icon preview"><?php echo _html_safe(PREVIEW); ?></button> <button type="submit" class="icon submit"><?php echo (!isset($bug['id'])) ? _html_safe(SEND) : _html_safe(UPDATE); ?></button></td></tr>
	</table>
</form>
