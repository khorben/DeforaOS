...
<?php } ?>
		...
		<tr><td></td><td><a href="<?php echo _html_link('wiki', FALSE, isset($wiki['id']) ? $wiki['id'] : FALSE, isset($wiki['id']) ? $wiki['title'] : FALSE); ?>"><button type="button" class="icon cancel"/><?php echo _html_safe(CANCEL); ?></button></a> <button type="submit" name="preview" class="icon preview" onclick="return editorPreview('wikicontent')"><?php echo _html_safe(PREVIEW); ?></button> <button type="submit" name="send" class="icon submit"><?php echo _html_safe(SUBMIT); ?></button></td></tr>
		</table>
	</form>
</div>
