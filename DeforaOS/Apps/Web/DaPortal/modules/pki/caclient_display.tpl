<div class="pki ca">
	...
	<div class="toolbar"><form action="<?php echo _html_link(); ?>" method="post"><input type="hidden" name="module" value="pki"/><input type="hidden" name="action" value="caclient_export"/><input type="hidden" name="id" value="<?php echo _html_safe($caclient['id']); ?>"/><input type="checkbox" name="private"><?php echo _html_safe(INCLUDE_PRIVATE_KEY); ?></input><br/><?php echo _html_safe(KEY); ?>: <input type="password" name="key" value=""/> <input type="submit" value="<?php echo _html_safe(EXPORT); ?>" class="icon submit"/></form></div>
</div>
