...
<?php } ?>
	<table>
		...
		<tr><td></td><td><a href="<?php echo _html_link('bookmark', FALSE, isset($bookmark['id']) ? $bookmark['id'] : FALSE, $bookmark['title']); ?>"><button type="button" class="icon cancel"><?php echo _html_safe(CANCEL); ?></button></a> <input type="submit" value="<?php echo isset($bookmark['id']) ? _html_safe(UPDATE) : _html_safe(SEND); ?>" class="icon submit"/></td></tr>
	</table>
</form>
