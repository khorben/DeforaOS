...
<div class="toolbar">
<a href="<?php echo _html_link('probe', '', isset($id) ? $id : '', '', ''); if(isset($time)) echo '?time='._html_safe($time); ?>" title="All">All</a>
<?php foreach($toolbar as $t) { ?>
&middot; <a href="<?php echo _html_link('probe', '', isset($id) ? $id : '', '', 'type='._html_safe($t['type'])); if(isset($time)) echo '&amp;time='._html_safe($time); ?>" title="<?php echo _html_safe($t['title']); ?>"><?php echo _html_safe($t['name']); ?></a>
<?php } ?>
</div>
<?php } if(isset($action)) { ?>
