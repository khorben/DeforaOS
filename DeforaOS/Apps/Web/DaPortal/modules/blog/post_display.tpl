...
<?php } else { ?>
		<a href="<?php echo _html_link('blog', FALSE, $post['id'], $post['tag']); ?>"><?php echo _html_icon('read'); ?> <?php echo _html_safe(READ); ?></a>
<?php if(_module_id('comment')) { ?>
		(<?php echo _html_safe(_module('comment', 'count', array('id' => $post['id'])).' '.COMMENT_S); ?>)
		<span class="middot">&middot;</span>
		<a href="<?php echo _html_link('blog', 'reply', $post['id']); ?>#edit"><?php echo _html_icon('reply'); ?> <?php echo _html_safe(REPLY); ?></a>
<?php } ?>
<?php global $user_id; require_once('./system/user.php');
if(isset($post['id']) && (_user_admin($user_id) || $user_id == $post['user_id'])) { ?>
		<span class="middot">&middot;</span>
		<a href="<?php echo _html_link('blog', 'update', $post['id']); ?>"><?php echo _html_icon('edit'); ?> <?php echo _html_safe(EDIT); ?></a>
<?php } } ?>
	</div>
</div>
